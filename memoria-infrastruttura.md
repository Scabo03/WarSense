...
5. **La versione dei VALORI non è discrezionale** (regola del titolare, 03 §9.2.1): si incrementa OGNI VOLTA che cambia una regola che incide sul modo in cui una partita in corso si svolgerebbe, e non soltanto quando cambia la forma dei file. Criterio pratico: se una partita salvata prima, riaperta dopo, proseguirebbe diversamente da come era cominciata, la versione sale — e sale nella stessa modifica che tocca la regola, insieme alle impronte. Vi rientrano le regole del combattimento, i costi, le soglie e ogni valore che entri in una formula del Motore; non vi rientrano le voci mai lette da una partita esistente. La versione dei TESTI resta ferma e discrezionale, perché le impronte del suo manifest bastano al rinfresco. Senza l'incremento, un salvataggio incompatibile non viene rifiutato e il giocatore ritrova una battaglia diversa da quella che aveva lasciato, senza alcun avviso: è già accaduto una volta, con la tranche dei modificatori di posizione.
6. **Le impronte** — ENTRAMBI i manifest le portano (per i testi dal 2026-08-03, RDA-54): è il cambiamento delle impronte a far rinfrescare la copia in Documenti, quindi chi modifica un file in `Contenuti/Valori/` o in `Contenuti/Testi/` DEVE rigenerare le impronte del rispettivo manifest nella stessa modifica, a versione ferma. Dimenticarlo significa installazioni con dati o testi stantii. Dal 2026-08-04 esiste lo script: `python3 scripts/rigenera-impronte.py`. Percorre ricorsivamente i due alberi, quindi raccoglie da sé anche i file in sottocartelle (per esempio `Valori/Mappe/`), riscrive le sole impronte e NON tocca mai le versioni, che restano discrezionali del titolare. Sostituisce il frammento Python scritto a mano che si usava prima.
...
Che cosa fa, nell'ordine: legge le credenziali dal file di configurazione locale; **controllo preventivo delle versioni** — interroga App Store Connect e rifiuta con motivo, prima di qualunque altra operazione e senza possibilità di aggiramento, una `MARKETING_VERSION` inferiore alla più alta già presente (regole 3 e 4 in testa al file); **controllo preventivo dei documenti consegnati** (`scripts/controlla-note.py`, che porta la TABELLA di quale documento è protetto da quali controlli: `note-di-rilascio.txt` per esistenza, lunghezza e freschezza; `nota-per-il-titolare-*.md` per esistenza, vocabolario, misure, cifre e freschezza); **controllo preventivo della corrispondenza con App Store Connect** (`scripts/controlla-build.py`: rifiuta se `build-caricate.md` e i server divergono, in entrambi i versi); ricava il numero di build (massimo su tutto l'account più uno); esegue il collaudo COMPLETO (`scripts/collaudo-completo.sh`: pacchetto, ospitate, interfaccia sul simulatore) e si ferma se fallisce; rigenera il progetto Xcode con `xcodegen generate` da `Applicazione/project.yml`; archivia (`xcodebuild archive`, configurazione Release, firma manuale con il certificato riusato e il profilo di cui sopra); esporta il pacchetto ipa (`xcodebuild -exportArchive` con `Applicazione/ExportOptions.plist`); carica con `xcrun altool` autenticato con la chiave API; attende l'elaborazione e allega la nota per i tester leggendola da `note-di-rilascio.txt` (se l'attesa scade: `./scripts/nota-testflight.sh <numero>` più tardi); **registra la build in `build-caricate.md`**, così che il passo umano che è mancato con la build 13 non esista più.
...
Quarto problema (build 7): la nota per i tester è stata rifiutata con `409 ENTITY_ERROR.ATTRIBUTE.INVALID.TOO_LONG` su `whatsNew`. App Store Connect accetta al massimo 4000 caratteri per quel campo; la nota ne aveva 4160. Il caricamento della build NON ne risente — era già andato a buon fine — e lo script lo dice, indicando di rieseguire `./scripts/nota-testflight.sh <numero>` più tardi. Soluzione applicata: nota accorciata a circa 3400 caratteri e riallegata con quel comando. Regola pratica: restare sotto i 4000 CARATTERI (non byte). **Dal 2026-08-04 la regola non è più affidata alla memoria: `scripts/carica-testflight.sh` conta i caratteri della nota PRIMA di compilare e rifiuta con motivo, come fa per la versione.** L'aggiunta è nata dal ripetersi dello stesso errore alla build 12, con la regola già scritta qui e dimenticata da chi la stava applicando: una regola si può dimenticare, un controllo no.

Nota di protezione già attiva: lo script esegue l'INTERO collaudo prima di archiviare e si ferma se una prova fallisce; non aggirarlo mai. «Intero» ha una sola definizione, `scripts/collaudo-completo.sh`, condivisa con l'integrazione continua: prove del pacchetto, prove ospitate e prove d'interfaccia sul simulatore. **Fino al 2026-08-05 questa riga diceva il falso in una parte e il vero in modo fuorviante nell'altra**: il collaudo si fermava a `swift test`, cioè alle prove del pacchetto, e la frase che seguiva — «i bersagli di prova del progetto applicativo non entrano nell'archivio, quindi non possono rompere una consegna» — presentava come protezione ciò che era il buco. Non potevano rompere una consegna perché non venivano mai eseguiti prima di una consegna, ed erano precisamente le prove al livello in cui i difetti riferiti dal titolare si sono manifestati (esame-critico.md §C4, §3.4). Resta vero, e senza contropartita, che quei bersagli non entrano nell'archivio: lo schema li dichiara solo per la fase di test.
