...
### RDA-122 — L'imboscata come ordine che si rinnova e consuma l'azione, modifica voluta (incide sul gioco; 01 §5.11, §5.6.0.5, §5.6.8.1, incarico 21)

Decisione del titolare: l'imboscata cessa di essere uno stato che DURA e diventa un ORDINE che si rinnova. È MODIFICA VOLUTA, non correzione di una svista. `ComandoCampagna.imboscata` consuma l'azione della giornata (`azioneSpesa` e `ordineImboscata` posti insieme), esattamente come marcia e presidio (01 §5.6.0.5); `haConclusoLaGiornata` NON include più `ordineImboscata` (era `azioneSpesa || inMarcia || ordineImboscata`, ora `azioneSpesa || inMarcia`). All'apertura della giornata `chiudiLaGiornataSeServe` AZZERA `ordineImboscata` con `azioneSpesa`: un gruppo appostato torna «in attesa» il mattino dopo e va RIAPPOSTATO. Il pannello offre «Mettiti in agguato» ogni giornata a un gruppo fermo, come il presidio (`SchermataMappaCampagna`). Questo SCIOGLIE ALLA RADICE la non terminazione dell'incarico 20 (uno stato di soli agguati faceva riaprire la giornata all'infinito): un appostato consuma l'azione e la cascata non può più correre. RIMOSSO il vecchio freno di `chiudiLaGiornataSeServe` (rompeva la cascata quando tutti conclusi e nessuno in marcia, RDA-119) — obsoleto, non serve più. RIMOSSA la revoca dell'imboscata (`ComandoCampagna.revocaImboscata`, evento `imboscataRevocata`, errore `gruppoNonInAgguato`, azione del pannello): con l'agguato per-giornata non c'è nulla da revocare (per non restare appostato non si ripete l'ordine), e l'elenco chiuso delle azioni di giornata (01 §5.6.8.1) non contiene alcuna revoca d'imboscata. SUPERA la parte di RDA-119 sulla persistenza dell'agguato e sul freno alla cascata. Invariante nuovo `appostato_senza_azione` col mutante; prova `RicognizioneImboscateTest`; il banco TERMINA (`test_incarico_21`, `test_05_12_6`).

### RDA-123 — L'occultamento dell'imboscata per RETROCESSIONE della conoscenza, non per eccezione alla visibilità (incide sul gioco; 01 §5.11.1, §12, incarico 21)

Decisione del titolare: un gruppo appostato non deve essere visibile a chi gli passa vicino, altrimenti l'imboscata non funziona; ma il gioco non dichiara MAI il falso (01 §12). La via è la CONOSCENZA che RETROCEDE da confermato, non un'eccezione alla visibilità. Realizzato come CAP a tempo di lettura dentro `MotoreCampagna.conoscenza(di:per:)`: se nella casella c'è un gruppo AVVERSARIO appostato che la parte non ha SCOPERTO, la sua conoscenza NON è mai confermato — si deriva dal solo RICORDO (memoria, presunto, inesplorato), e un ricordo ancora fresco (confermato per età) retrocede al limite dell'avvistato (`avvistato(turni: soglia)`). Il gioco non annuncia mai «vuoto» né «confermato»: annuncia «avvistato» (notizia non più certa) o meno. La proiezione (`VistaCampagna.vociDiCasella` mostra l'occupante avversario solo dove confermato) e la vista dell'avversario (`vistaAvversario` note-building) SEGUONO dal cap senza eccezioni. **Come si evita il falso nell'istante in cui il gruppo si apposta mentre l'altra parte lo osserva:** il cap sostituisce l'osservazione corrente col ricordo nello STESSO momento in cui l'ordine è dato — chi osservava la casella passa da confermato ad avvistato (turni = soglia) atomicamente; il gioco non afferma mai il vuoto né mostra l'appostato, dichiara solo minor certezza, che è VERA perché il gruppo si è nascosto. Non muta lo stato memorizzato (compatibile con `conoscenza_regredita_senza_tempo`, che confronta il RICORDO fra due stati). Vale SIMMETRICAMENTE (party-agnostico): l'avversario decide sulla propria conoscenza, non vede gli agguati occulti del giocatore e vi può cadere. Invariante nuovo `occultamento_violato` col mutante (sonda che riceve la conoscenza del nemico dall'esterno, così un mutante può iniettare il falso «confermato»); prova `RicognizioneImboscateTest`.

### RDA-124 — La scoperta dell'imboscata riservata alla ricognizione (incide sul gioco; 01 §5.11.1, §5.4, incarico 21)

Il SOLO modo di scoprire un'imboscata pendente sono le formazioni di RICOGNIZIONE, alle condizioni già realizzate per l'esplorazione (01 §5.4, RDA-117): la riuscita discende dalla competenza contro l'insidiosità, senza estrazione, e il tentativo può risolversi anche con gli esploratori notati o perduti. Un'esplorazione RIUSCITA che riveli l'area (`rivelaArea`) scopre ogni gruppo avversario appostato che vi si trova (`scopriLeImboscate`): la casella entra in `StatoCampagna.imboscateScoperte: [Parte: Set<Cella>]`, che LEVA l'occultamento (`conoscenza` la torna confermato e la parte vi vede l'appostato), e il fatto entra nel registro col luogo, attivabile — evento `EventoCampagna.imboscataScoperta(parte:casella:)` e fatto `FattoRegistrato.imboscataScoperta(casella:)`, con `registro.imboscata_scoperta`/`campagna.imboscata_scoperta` nei testi; consegnato al solo giocatore che scopre (`proiettaPerIlGiocatore`, party). `imboscateScoperte` si azzera all'apertura della giornata come l'ordine di imboscata: ogni giornata è una nuova imboscata da scoprire. Un gruppo armato che passa accanto NON se ne accorge — l'occultamento tiene contro la sola osservazione. La «insidiosità dell'imboscata» NON è un dato nuovo: un gruppo appostato conta già come nemico armato vicino (`peso_nemici_vicini`, RDA-117), sicché rende la zona più insidiosa da esplorare senza introdurre numeri. La CONDOTTA dell'avversario avanza in forma MINIMA e PROVVISORIA la tattica d'agguato rinviata in S18/S19 (un armato che non guadagna avanzando e ha una formazione nota adiacente si apposta): soglia e raggio restano da tarare col gioco. Fatto nuovo `imboscata_scoperta`; invarianti `partita_non_terminata` (termina entro le giornate dichiarate — cancello che FALLISCE, non freno che nasconde) col mutante; prove `RicognizioneImboscateTest`, `test_incarico_21`.

### RDA-125 — Perché l'avversario non si vedeva: la TARATURA del raggio di osservazione, non un difetto né un falso (incide sul gioco; 01 §5.6.11, §5.3, incarico 22)

Accertamento delle due osservazioni del titolare sulla build 23, prima di ogni intervento. **Nessun falso.** L'avvistamento avversario nasce SOLO dove il giocatore osserva: `MotoreCampagna` produce `EventoCampagna.formazioneAvversariaAvvistata` unicamente per gli arrivi avversari su caselle `osservata(da: .giocatore)`, e la voce d'annuncio dell'occupante avversario (`VistaCampagna.vociDiCasella`) compare solo dove la conoscenza è `confermato`. Il numero «avvistato, N turni» che il titolare sentiva interrogando il proprio quartier generale NON è un nemico: è lo STATO DI CONOSCENZA della casella (`StatoConoscenza.avvistato(turni:)`, `conoscenza.avvistato` = «avvistato, %#@turni@»), cioè l'età del suo ultimo sguardo su quella casella, invecchiata perché s'era allontanato — l'unica grandezza che produce un numero di turni; l'occupante avversario non porta mai turni. **La causa reale era la TARATURA:** `raggio_osservazione` = **1** su una mappa di cento caselle fa vedere pochissimo, e l'avversario — che si muove per corsie diverse (aggiramento, RDA-114) — non entrava mai nel campo di un passo del giocatore. Misura col banco (`BancoCampagna`, campo nuovo `Corsa.avvistamenti`, prova `test_incarico_22_il_banco_genera_e_misura_gli_avvistamenti`), raggio **1** → **2**, su quaranta giornate: scenario `pianura_contro_avversario` (dieci per dieci) da **3** avvistamenti (primo alla giornata **10**, mappa osservata **32%**) a **14** (primo alla giornata **8**, **43%**); `guado_contro_avversario` da **7** a **11**; `ricognizione_imboscate_pianura` da **5** a **19**. L'informazione resta INCOMPLETA (a raggio 2 la `pianura` osserva il 43%, il resto no): l'avversario si manifesta, non è sempre visibile — il criterio dell'incarico. NON è un difetto del codice, NON è la condotta (l'avversario incrocia il giocatore, semplicemente non lo si vedeva). `raggio_osservazione` **1→2** in `conoscenza-campagna.json`; `raggio_esplorazione` **2→3** in `ricognizione-campagna.json` (il caricatore esige `raggio_esplorazione > raggio_osservazione`, `ricognizione_incoerente`, o esplorare non rivelerebbe nulla di nuovo). Prove aggiornate alla nuova taratura: `ConoscenzaTest.test_5_3` (distanza due ora confermata, tre inesplorata), `RicognizioneImboscateTest` (l'esplorazione rivela oltre il raggio ordinario, distanza tre). Valori PROVVISORI, da tarare giocando.

### RDA-126 — Ogni formazione dichiara la CATEGORIA; la propria dichiara anche la COMPOSIZIONE (incide sul gioco; 02 §3.8.1, §6.4.1, incarico 22)

Ogni formazione dichiara CHE COSA È, non solo come si chiama. La CATEGORIA (gruppo armato, ricognizione, non armata) era già dichiarata dall'incarico 19 — per le proprie con la chiave d'occupante secondo la categoria (`casella.occupante_proprio`/`_ricognizione`/`_non_armata`), per le avversarie con `casella.occupante_avversario*`, e con il marcatore di FORMA sulla mappa uguale per le due parti (`»` ricognizione, `≈` non armata, nulla l'armato ordinario) — sicché ciò che si vede coincide con ciò che si sente. Questo incarico aggiunge, per le SOLE proprie formazioni, la COMPOSIZIONE: di quali reparti e di quanti atomi è fatta. Voce nuova `VistaCampagna.VoceDiCasella.composizionePropria([Reparto])`, prodotta da `vociDiCasella` DOPO tutte le voci di testa (la categoria propria e quella avversaria comprese), sicché è un dettaglio di bassa priorità che il livello sintetico lascia cadere per primo dalla coda (02 §3.8.1). L'annuncio riusa la frase «N atomi di <archetipo>» della divisione (`divisione.reparto`, dallo stringsdict) unita in elenco dietro `casella.composizione` = «composta da %@»: NESSUN termine nuovo del vocabolario chiuso (S18). La composizione NON esiste per le avversarie, di cui non si dichiara il volume (02 §6.4.1): appartiene al solo occupante proprio. Non ha SEGNO sulla mappa — l'identità (l'iniziale del nome) e la categoria (il marcatore) bastano al colpo d'occhio, il numero degli atomi non ha una forma — sicché `segno(di: .composizionePropria)` è `nil`; la regola «il segno coincide con l'annuncio» riguarda le tre CATEGORIE, già distinte per forma. Prova d'interfaccia `AnnuncioDiCasellaTest.test_incarico_22_la_propria_formazione_dichiara_la_composizione` (normale e dettagliato la dichiarano, sintetico la taglia); la struttura del taglio dalla coda regge (`test_00_9_5`).

### RDA-127 — I due cancelli dell'avvistamento: che l'avversario si manifesti, e che nessun avvistamento sia falso (incide sul collaudo; 01 §5.6.11, incarico 22)

Due invarianti nuovi in `SondaInvariantiCampagna`, ciascuno col mutante in `tavolaDeiMutanti` (guardia `test_incarico_6_ogni_invariante_ha_almeno_un_mutante`). **`nessun_avvistamento_in_partita`**: in una partita INTERA contro l'avversario il giocatore deve avvistare almeno una formazione avversaria — il difetto che il collaudo non aveva colto e che il titolare vide giocando, reso un cancello che FALLISCE. `controllaAvvistamentoAvvenuto(avvistamenti:conAvversario:)` riceve il conteggio dall'esterno (lo produce il banco lungo la corsa), come `controllaTerminazione`. Vale SOLO sulle partite complete: `BancoCampagna.corri` prende `partitaCompleta: Bool = true`, e il fumo — che tronca a poche giornate — passa `false` (`ProgrammaDiVerifica`), perché una corsa troncata può legittimamente non aver ancora avuto il primo avvistamento (che arriva alla giornata 5–8). **`avvistamento_senza_formazione`**: a un avvistamento dichiarato deve corrispondere una formazione avversaria REALMENTE su quella casella osservata, o il gioco dichiarerebbe il falso — la violazione più grave. In `controllaRegistro` la voce `formazioneAvversariaAvvistata` è ora distinta: se la casella non è osservata è fuga d'informazione (`registro_rivela_ignoto`, già esistente); se è osservata ma senza avversario è `avvistamento_senza_formazione`. Il banco chiama `controllaRegistro` anche dopo il turno del GIOCATORE (prima solo dopo l'avversario), perché la cascata di fine giornata e i suoi avvistamenti possono chiudersi sull'ultimo comando del giocatore. Misura permanente nella `Corsa` (`avvistamenti`, `primoAvvistamento`, `gapMedioAvvistamenti`, `caselleOsservate`/`caselleTotali`), riportata a ogni corsa (`FENOMENI-22`). Prove `test_incarico_22_il_cancello_del_nessun_avvistamento_scatta_e_tace`, `test_incarico_22_il_cancello_dell_avvistamento_senza_formazione_scatta_e_tace`.

### RDA-128 — La causa vera per cui il giocatore non incontrava l'avversario: lo SCENARIO GIOCABILE, non il raggio (incide sul gioco; 01 §5.2, §5.6.11, incarico 23)

Accertamento, prima di ogni intervento, dei tre fatti riferiti dal titolare giocando la build 24 (esploratori assenti in partita, avversario mai avvistato su sedici caselle, composizione propria che funziona). **La causa non era la taratura del raggio** (la diagnosi dell'incarico 22 era incompleta): la ricognizione, l'avversario e le formazioni non armate erano stati costruiti nel Motore (`CategoriaFormazione`, `gruppiAvversario`, la condotta, le azioni `esplorazione`/`sabotaggio`/`studioApprofondito`) e provati al banco su `campagne.json`, ma lo scenario che il giocatore APRE — `Codice/Sources/Contenuti/Valori/scenari-campagna.json`, letto da `PartitaCampagna.scenario(_:)`, i tre formati `campagna_piccola`/`media`/`grande` — conteneva SOLTANTO gruppi armati del giocatore, senza `gruppi_avversario`, senza ricognizione, senza non armate. Il programma di verifica non l'aveva colto perché genera i propri scenari (`campagne.json`, cartella `Scenari/Campagne`) e non guarda il file giocabile: due sorgenti diverse.

**Dove sta la causa, coi numeri, punto per punto (come chiede l'incarico).** *Se le formazioni avversarie esistano nello scenario:* NO — il nuovo invariante `controllaCategorieScenario` (RDA-130), applicato al file PRIMA della correzione (`swift test` sul cancello), riportava per ciascuno dei tre scenari le cinque categorie mancanti: `categoria_mancante_nello_scenario:scenario=campagna_piccola:parte=avversario:categoria=armato/ricognizione/non_armata` e `…:parte=giocatore:categoria=ricognizione/non_armata` (idem per media e grande). Zero gruppi avversari. *Dove si trovino all'avvio, se si muovano:* non esistendo, non si trovavano da nessuna parte e la condotta non muoveva nulla — provato dal fatto che `CatenaInterfacciaMotoreTest.test_00_3_1`, sullo scenario vecchio, registrava nel giornale esattamente UN comando per ciascun tocco del giocatore (nessun comando d'avversario); dopo la correzione lo stesso test registra i comandi del giocatore PIÙ quelli della condotta avversaria (su `campagna_piccola`, quattro giornate: 9 tocchi del giocatore → 19 comandi nel giornale, cioè 9 del giocatore + 10 dell'avversario). *Se l'osservazione le rilevi:* il raggio 2 (incarico 22) era corretto, ma non c'era nulla da rilevare. *Se l'annuncio le dichiari:* la macchina dell'annuncio (`VistaCampagna.vociDiCasella` → `casella.occupante_avversario*`, RDA-126) esisteva e funzionava, ma non riceveva mai un avversario. **I tre esiti possibili (codice difettoso / taratura / ipotesi sbagliata): nessuno dei primi due — il difetto era il DATO** (lo scenario giocabile non usava le capacità costruite); l'ipotesi dell'incarico era GIUSTA.

**Correzione — lo scenario iniziale schiera ora le tre categorie per entrambe le parti** (in `scenari-campagna.json`). I gruppi armati preesistenti del giocatore restano IDENTICI e nello stesso ordine (gli identificatori, e quindi `gruppiOrdinati[0]`, non si spostano); si AGGIUNGONO, in coda, un esploratore e una formazione non armata; l'avversario schiera, presso il proprio quartier generale, un gruppo armato, un esploratore e una formazione non armata. Composizione e criterio: **`campagna_piccola` (guado, 4×4)** giocatore = 2 armati (invariati) + 1 ricognizione (competenza 5) + 1 non armata (carico 4, soglia 2) = 4; avversario = 1 armato + 1 ricognizione (competenza 5) + 1 non armata (carico 4, soglia 2) = 3. **`campagna_media` (istmo, 6×6)** giocatore = 3 armati + 1 ricognizione + 1 non armata = 5; avversario = 3 (1+1+1). **`campagna_grande` (pianura, 10×10)** giocatore = 5 armati + 1 ricognizione + 1 non armata = 7; avversario = 3 (1+1+1). *Criterio:* ogni parte schiera almeno un gruppo di ciascuna categoria su ogni mappa; il giocatore conserva la sua forza armata di prima e vi aggiunge un esploratore e una non armata; l'avversario, presso il suo quartier generale (righe alte), ne schiera uno per categoria. Le due parti partono dalle metà opposte, sicché con raggio di osservazione 2 l'incontro esige una marcia o la condotta avversaria — la ricognizione resta il modo di sapere — ma è garantito (provato: `EsploratoriAvversarioInterfacciaTest.test_incarico_23_il_giocatore_avvista_l_avversario_e_la_casella_e_arancione`, che marciando avvista l'avversario su `campagna_piccola`). Le competenze/carichi/soglie sono PROVVISORI. Impronte dei Contenuti rigenerate (`rigenera-impronte.py`); versione dei valori INVARIATA (0.11.0), lo scenario è un DATO, non un valore tarato.

### RDA-129 — Il segno del nemico sulla mappa: il riempimento ARANCIONE, cue di parte ridondante col segno di forma (incide sul gioco; 02 §2.8, 00 §1.2, incarico 23)

Una casella con una formazione avversaria AVVISTATA si riempie interamente di arancione, esattamente come quella di un proprio gruppo si riempie di blu — stessa estensione, cambia il colore. Realizzato nel blocco che DISEGNA il riempimento (`SchermataMappaCampagna`, chiusura `vistaMappa.coloreCasella`): un ramo nuovo `if costruttore.vista.avversarioAvvistato(su: casella) { return .systemOrange }`, PRIMA del blu (la compresenza 01 §6.1 è possibile e la minaccia va segnalata; la lettera centrale del proprio gruppo e i segni di forma si disegnano comunque sopra il riempimento). La verità della cosa non si calcola nella Presentazione (00 §3.2): `VistaCampagna.avversarioAvvistato(su:)` (nuovo, in `InterrogazioniCampagna`) DERIVA dal medesimo elenco `vociDiCasella` — è vero sse quell'elenco contiene una `.occupanteAvversario` — sicché il riempimento e l'annuncio non possono divergere (dove l'uno dichiara l'avversario, l'altro lo colora), e come la voce è vero solo dove la conoscenza è `confermato` (l'occultamento regge). **Il colore non distingue le CATEGORIE:** quello lo fa la FORMA — i segni «×» (armato avversario), «×»» (ricognizione avversaria), «×≈» (non armata avversaria) restano (`CostruttoreAnnunciCampagna.segno(di:)`); l'arancione dice soltanto «nemico», ridondante con la «×». Le categorie non sono mai distinte per il solo colore (regola dell'incarico rispettata). Prova d'interfaccia sulla schermata vera (nuovo attrezzo `SchermataMappaCampagna.coloreCasellaPerProva`, che legge lo STESSO blocco del disegno): `EsploratoriAvversarioInterfacciaTest.test_incarico_23_…arancione` accerta, sullo scenario giocabile aperto come lo apre il giocatore, che la casella dell'avversario avvistato è `.systemOrange`, porta la «×» (`segniCasella`) e si annuncia «…avversari…».

### RDA-130 — L'invariante dello scenario iniziale, col mutante: le tre categorie per entrambe le parti (incide sul collaudo; 01 §5.2, incarico 23)

Invariante nuovo in `SondaInvariantiCampagna`: `controllaCategorieScenario(nome:scenario:)` pretende che uno scenario di campagna GIOCABILE schieri, per entrambe le parti, un gruppo armato, una formazione di ricognizione e una non armata; ogni categoria mancante per una parte è una violazione `categoria_mancante_nello_scenario:scenario=…:parte=…:categoria=…`. È il difetto che ha reso inutili due build, reso ora un cancello che FALLISCE invece di un'omissione silenziosa. Riceve lo scenario dall'esterno (come `controllaTerminazione`/`controllaAvvistamentoAvvenuto`), così da giudicare i tre scenari giocabili di `scenari-campagna.json` senza rifarli. Codice aggiunto a `codiciNoti`; mutante in `tavolaDeiMutanti` (uno scenario di soli armati, senza avversario), sicché la guardia `test_incarico_6_ogni_invariante_ha_almeno_un_mutante` pretende comunque un mutante. **La garanzia vera** è nella prova `InvariantiCampagnaTest.test_incarico_23_lo_scenario_iniziale_schiera_le_tre_categorie`, che DECODIFICA il file giocabile (`Contenuti.valoriDiFabbrica/scenari-campagna.json`, `[String: ScenarioCampagna]`), pretende tre scenari, e per ciascuno zero violazioni per entrambe le parti — il cancello che rende impossibile ripetere il difetto. Visto FALLIRE di proposito sul file PRIMA della correzione (`swift test`, uscita: le cinque categorie mancanti per ciascuno dei tre scenari) e la guardia dei mutanti PASSATA nella stessa corsa; verde dopo la correzione (33 prove, 0 fallite).

### RDA-131 — La forma del passaggio fra i due piani: innesco al contatto, blocco all'accettazione, apertura per scelta (incide sul gioco; 01 §6, §9.4.1, §15, incarico 24)

Il pezzo che unisce le due metà del gioco. **Innesco (01 §6.1, §6.1.1):** alla RISOLUZIONE DI FINE GIORNATA, un gruppo armato che ENTRA (è fra gli `arrivati`) in una casella occupata da un armato contrapposto impone lo scontro — «chi arriva sceglie tempo e luogo; chi attendeva riceve la prima mossa». Realizzato in `MotoreCampagna.innescaLeBattaglie(_:arrivati:)`, gemello di `scattaLeImboscate` e chiamato subito dopo di esso in `risolviFineGiornata` (l'agguato ha la precedenza sulla stessa casella, col suo vantaggio). Un contatto crea un `BattagliaInSospeso` (nuovo, in `StatoCampagna`): casella, i due gruppi, `primoOccupante` (chi NON è arrivato — l'ordine dei turni di 01 §9.4.1), `imboscante` (nil per l'ordinaria, la parte che imboscava per l'agguato), giorno, e un `identificatore` DETERMINISTICO dalla casella e dal giorno (`battaglia-giorno-riga-colonna`) che nomina lo slot su disco. **Blocco (01 §6.3, §6.4):** finché `stato.battaglieInSospeso` non è vuoto, `MotoreCampagna.valida` respinge OGNI comando col medesimo motivo `battaglia_in_sospeso` (nuovo, `MotivoNonValidoCampagna`), e la cascata di fine giornata (`chiudiLaGiornataSeServe`) e il turno dell'avversario (`SessioneCampagna.svolgiTurnoAvversario`) si fermano — «nulla avanza in quella campagna». **Apertura (01 §6.2, 02 §5.6):** il passaggio alla schermata di battaglia NON è forzato; nella casella compare il comando «Apri la battaglia» (`SchermataMappaCampagna.apriPannello`, `pannello.apri_battaglia`), che il giocatore decide di attivare — il fuoco non si sposta se non lo chiede. **Ritorno (01 §15.4, §15.5, §10.6, §15.2.3):** `MotoreCampagna.applicaEsitoInCampagna` piega l'esito sulla mappa — il vincitore resta nella casella contesa, lo sconfitto arretra verso il proprio quartier generale (`PonteCampagnaBattaglia.casellaArretrata`, «arretrata rispetto alla direzione di provenienza» reso come «verso casa», la mappa non tiene la provenienza — S24), un gruppo annientato SPARISCE, e la campagna si sblocca. La conversione fra i due piani vive nel Motore, non nella Presentazione (`PonteCampagnaBattaglia`), perché è regola e la condivide il programma di verifica.

### RDA-132 — I due giornali che si toccano: la battaglia nel proprio slot, l'esito iscritto nel giornale di campagna (incide sul formato di salvataggio; 05 §6.1, §6.3, 00 §15.2, incarico 24)

Battaglia e campagna vivono in slot distinti, e il giornale è il formato di salvataggio: il passaggio li fa toccare. **Andata:** aprendo la battaglia, `PartitaCampagna.apriBattaglia` costruisce lo `ScenarioBattaglia` dai due gruppi (`PonteCampagnaBattaglia.scenario`: deck = composizione, un elemento per reparto) e apre un `PartitaCorrente(daCampagna:cartella:identificatore:)` in una sottocartella dedicata dello slot di campagna (`campagna-di-prova/battaglie/<identificatore>/`), col PROPRIO giornale. Se lo slot esiste già lo RIPRENDE (una battaglia chiusa a metà e riaperta è la stessa, 05 §6.3): è così che una battaglia nata dalla campagna si salva, si riprende e si rigioca dopo un riavvio — verificato spegnendo e riaprendo (`PassaggioBattagliaInterfacciaTest.test_incarico_24_l_esito_torna_in_campagna_dopo_un_riavvio`, che chiude la Sessione della battaglia e la riapre dallo slot, impronta identica). **Ritorno:** conclusa la battaglia, `SessioneCampagna.concludiBattaglia` iscrive nel giornale di CAMPAGNA una voce nuova `VoceGiornale.battagliaConclusa(EsitoInCampagna)` — è il punto in cui i due giornali si toccano — e la piega sullo stato. `EsitoInCampagna` è un DATO autosufficiente (superstiti già raggruppati per archetipo, caselle del ritorno, chi è annientato): rigiocare il giornale di campagna riproduce il ripiegamento IDENTICO senza rileggere i file della battaglia (`ricostruisci` gestisce la voce con `applicaEsitoInCampagna`), sicché l'esito torna in campagna anche dopo un riavvio dell'applicazione. La battaglia rigiocata dal PROPRIO giornale è a sua volta identica (deterministica, nessuna estrazione): invariante `rigiocatura_battaglia_divergente`. **Compatibilità (00 §15.2):** lo schema del giornale di campagna sale da 5 a 6 (le regole di fine giornata sono cambiate: un contatto armato che prima non produceva nulla ora innesca una battaglia); i salvataggi di versione 5 si DICHIARANO incompatibili invece di riaprirsi in silenzio con una storia alterata (`SessioneCampagna.init(riprendi:)` → `schemaIncompatibile`, annunciato da `SchermateContorno.riprendiCampagna` con `campagna.slot_schema_incompatibile`). Il caso nuovo del giornale ha il proprio campione committato (`CampioniGiornale/campioni.jsonl`, riga 29), che si ricodifica byte per byte identico (`CompatibilitaGiornaleTest`).

### RDA-133 — Il vantaggio dell'imboscante, raccolto e realizzato (incide sul gioco; 01 §9.3.2, §5.11.2, incarico 24, chiude RDA-98)

Una sessione precedente aveva registrato lo scatto dell'imboscata nello stato (`ImboscataInSospeso`, con casella, imboscante e intruso) senza costruire il vantaggio, perché apparteneva alla battaglia (RDA-98, «il passo già preparato»). Adesso la battaglia c'è. `scattaLeImboscate` crea, oltre all'`ImboscataInSospeso` invariata (che resta il FATTO di registro e l'occultamento), un `BattagliaInSospeso` con `imboscante` = la parte appostata, che occupava per prima e agisce per prima (01 §9.4.1). Aprendo quella battaglia, `PonteCampagnaBattaglia.scenario` mette `imboscata: true` nello `ScenarioBattaglia`, da cui `FabbricaBattaglia` accende `sorpresa: .vantaggio(restanti: f.turniVantaggioImboscante)` — il vantaggio già COSTRUITO sul lato battaglia (turni consecutivi in più e sconto del 30% sul piazzamento, 01 §9.3.2, `FormatoBattaglia.turniVantaggioImboscante`/`scontoImboscante`): il ponte è la sola cosa che mancava. Che una battaglia da imboscata si comporti DIVERSAMENTE da una ordinaria è verificato: il banco distingue le battaglie `daImboscata` (che portano `sorpresa: .vantaggio`) da quelle ordinarie (`sorpresa: .trasparente`), e ne conta gli esiti (`Corsa.battaglieDaImboscata`); su `guado_contro_avversario` almeno una battaglia nasce da agguato. Perché il banco le GENERI, la sua condotta tende un'imboscata quando un armato avversario è adiacente (`armatoAvversarioAdiacente`) invece di marciargli sopra: così l'ingresso dell'avversario apre una battaglia da agguato.

### RDA-134 — Gli invarianti del passaggio, ciascuno col mutante, e il banco che gioca le battaglie (incide sul collaudo; 01 §6, §15, incarico 24)

Quattro invarianti nuovi in `SondaInvariantiCampagna`, ciascuno col mutante in `tavolaDeiMutanti` (guardia `test_incarico_6_ogni_invariante_ha_almeno_un_mutante`): **`forza_non_conservata_nel_passaggio`** — gli atomi che tornano in campagna per ciascuna parte coincidono coi superstiti della battaglia (sciami in campo più riserve del deck), contati indipendentemente dal Ponte: nessuna forza si perde o si duplica nel passaggio. **`ritorno_in_campagna_scorretto`** — un gruppo annientato è sparito dalla mappa, un superstite c'è con la composizione ridotta e alla casella dovuta (vincitore nella contesa, sconfitto arretrato). **`blocco_battaglia_non_effettivo`** — con una battaglia in sospeso un comando è respinto col motivo dovuto, senza non lo è. **`rigiocatura_battaglia_divergente`** — l'esito di una battaglia rigiocata dai suoi comandi è identico (impronte a confronto). Il banco `BancoCampagna` non RENDE soltanto possibili le battaglie: le GENERA e le gioca (`giocaBattaglieInSospeso`), con due tattici deterministici come `BancoSessioniBattaglia`, sorvegliando i quattro invarianti; e ne riporta il numero, l'esito e quante da imboscata (`Corsa.battaglieGiocate/Vinte/Perse/DaImboscata`, stampa `FENOMENI-21 … BATT`). Una partita può ora concludersi con l'annientamento di una parte molto prima del limite (01 §15.2.3), esito normale: le prove del banco lo ammettono (`corsa.giornate >= limite || battaglieGiocate > 0`). Prova d'interfaccia sul simulatore che apre la campagna, porta due gruppi a contatto, apre la battaglia dal comando della casella, la combatte a conclusione e verifica sulla schermata vera il ritorno (`PassaggioBattagliaInterfacciaTest`); vista FALLIRE di proposito neutralizzando l'innesco (nessuna battaglia in sospeso → «in 60 giornate i due armati non sono mai entrati in contatto», TEST FAILED).

### RDA-135 — La causa vera del blocco dopo la battaglia: il combattente tornava NON-AGITO (incide sul gioco; 01 §5.6.0.5, §5.6.0.6, incarico 25)

Il titolare, giocando la build 26, ha trovato la campagna BLOCCATA in modo irreversibile al ritorno dalla battaglia: il gruppo vincitore restava nella casella, «né agito né in attesa», la giornata non si chiudeva perché il gruppo risultava non avere agito, e non c'era modo di farlo agire. **La causa vera:** `MotoreCampagna.applicaEsitoInCampagna` (incarico 24) riportava i superstiti in campagna con `azioneSpesa = false`, cioè NON-AGITI. Ma un gruppo che ha combattuto ha speso la giornata combattendo (01 §5.6.0.5: «ogni azione consuma l'intera giornata del gruppo che la compie … uno che si mette in imboscata non fa altro»): tornare non-agito lo mette in uno stato che 01 §5.6.0.6 dichiara impossibile — «un gruppo che non ha agito è sempre un gruppo che attende una decisione», cioè ordinabile. La giornata, che si chiude automaticamente solo quando TUTTI i gruppi hanno agito (01 §5.6.0.6), restava aperta in attesa del combattente, che il giocatore doveva ordinare con un ordine di troppo — e nel flusso reale non poteva.

**La correzione, per costruzione (non un tappo su un caso particolare, come l'incarico esige):** il gruppo che ha combattuto SPENDE la giornata combattendo — `applicaEsitoInCampagna` riporta i superstiti (di ENTRAMBE le parti) con `azioneSpesa = true`. Il combattente risulta AGITO, la giornata si chiude di conseguenza (`applicaEsitoInCampagna` chiama ora `chiudiLaGiornataSeServe` subito dopo il ripiegamento), e il giocatore non deve dare alcun ordine al reduce. Prova d'interfaccia `PassaggioBattagliaInterfacciaTest.test_incarico_25_…`, che apre la partita come il giocatore, porta a contatto, combatte ATTRAVERSO LA CATENA VERA dello schermo (comando della casella, schermata di battaglia, resoconto e suo congedo), torna in campagna e PROSEGUE due giornate intere vedendo le giornate chiudersi: FALLISCE sulla build 26 con «il gruppo … reduce dalla battaglia deve aver SPESO la giornata combattendo» (il superstite ha `azioneSpesa=false`), verde dopo la correzione. **Dichiarato non verificato:** il blocco PERMANENTE esatto non è stato riprodotto in una prova automatica — le battaglie naturali di `campagna_piccola` annientano il gruppo del giocatore (nessun combattente superstite del giocatore), e uno scenario costruito in cui il giocatore vince produce, col tattico attuale, una battaglia che non conclude (avversario trattenuto che non schiera); la prova coglie la causa alla radice (`azioneSpesa` del combattente) e il proseguimento oltre il ritorno.

### RDA-136 — Il ripiegamento dello sconfitto: verso il quartier generale, scelto dal giocatore, con la direzione del taglio (incide sul gioco; 01 §10.6, §15.5, §5.2.2.2, incarico 25)

Lo sconfitto non annientato ripiega di UNA casella all'indietro (01 §10.6, §15.6), e il giocatore SCEGLIE subito quale, a battaglia appena conclusa. **La direzione retrostante è quella del TAGLIO del rifornimento, non una seconda nozione** (01 §5.2.2.2: «retrostante significa dalla parte del proprio quartier generale»): `PonteCampagnaBattaglia.caselleDiRipiegamento` usa `MotoreCampagna.caselleAlleSpalle` — le caselle alle spalle secondo la stessa geometria del taglio — ristrette a quelle STRETTAMENTE più vicine al proprio quartier generale (un passo all'indietro, mai un avanzamento: 01 §10.6) e libere da un proprio gruppo (una formazione per parte per casella). Sostituisce l'euristica «vicino più vicino al QG» dell'incarico 24 (`casellaArretrata`), che era una seconda nozione di direzione (S25c). **La scelta è del giocatore, in forma praticabile ascoltando** (02 §8): `SchermataMappaCampagna.scegliRipiegamento` presenta le caselle disponibili, ciascuna come voce a sé in una frase compatta («In riga R, casella C»), e si sceglie attivandola — nessuna tabella, nessun trascinamento, nessun congedo (lo sconfitto DEVE ripiegare, sceglie solo dove). La scelta entra nell'`EsitoInCampagna` che il giornale di campagna registra, sicché è deterministica alla rigiocatura. La condotta avversaria e il default prendono la prima casella. **Il vincitore resta** nella casella contesa (01 §15.5); l'eccezione dei documenti — il vincitore che ripiega avendo chiamato ritirata dopo l'avversario (01 §15.2.2, «entrambi hanno chiamato ritirata») — è IRRAGGIUNGIBILE col motore attuale, che respinge una seconda `dichiaraResa` (`MotoreBattaglia.valida` riga 325, `resaNonDisponibile`): una sola parte può chiamare ritirata ed è sempre lo sconfitto (S25b). **Caso limite chiuso e dichiarato (S25a):** se in direzione del quartier generale non esiste alcuna casella disponibile — al bordo della mappa o con le spalle occupate — lo sconfitto RESTA nella casella contesa (compresenza col vincitore, una formazione per parte, 01 §6.1): non si può creare una casella dove non c'è, e non si può sovrapporre due gruppi della stessa parte.

### RDA-137 — L'invariante della giocabilità: nessun gruppo non-agito senza azioni (incide sul collaudo; 01 §5.6.0.6, incarico 25)

Invariante nuovo in `SondaInvariantiCampagna`: `controllaGiocabilita(stato:ordinabile:)` pretende che, quando la campagna NON ha una battaglia in sospeso, ogni gruppo che NON ha agito e NON è in marcia sia ORDINABILE — esista per esso almeno un comando di campagna valido (`gruppo_bloccato_senza_azioni` altrimenti). È lo stato del blocco irreversibile reso impossibile per costruzione, non evitato per disciplina. Una battaglia in sospeso è ESCLUSA perché è un blocco LEGITTIMO e temporaneo, che il giocatore scioglie aprendo la battaglia dalla casella (01 §6.2), non irreversibile. La validità dei comandi arriva dall'esterno (`ordinabile`), come gli altri invarianti che non rifanno il Motore; il banco la calcola provando presidio, sosta con raccolta, le marce ai vicini e — secondo la categoria — esplorazione o imboscata (`BancoCampagna.ordinabile`). Codice in `codiciNoti`; mutante in `tavolaDeiMutanti` (uno stato senza battaglia in sospeso con `ordinabile` sempre falso), sicché la guardia `test_incarico_6_ogni_invariante_ha_almeno_un_mutante` lo pretende. Il banco lo controlla dopo ogni comando e SUBITO DOPO IL RITORNO dalla battaglia (`giocaBattaglieInSospeso`), che è dove il difetto viveva. Nota: poiché la sosta con raccolta è sempre valida per un gruppo non-agito (`MotoreCampagna.valida`), un ben formato stato SBLOCCATO non ha mai un gruppo bloccato — l'invariante è quindi la RETE che coglie una regressione (una battaglia in sospeso persistente, o una regola futura che tolga la sosta), e il difetto del combattente non-agito lo coglie invece la prova d'interfaccia (`azioneSpesa` del reduce). **Il banco PROSEGUE dopo le battaglie:** nuovo scenario `pianura_vince_e_prosegue` in cui il giocatore VINCE e la campagna prosegue; misura `Corsa.giornateDopoLaPrimaBattaglia`, riportata (`test_incarico_25_il_banco_prosegue_dopo_le_battaglie`): media 18, massimo 33 giornate dopo la prima battaglia (dallo strumento), zero violazioni di giocabilità.

### RDA-138 — Il comando di chiusura della giornata: modifica VOLUTA della regola che la giornata si chiude solo da sé (incide sul gioco; 01 §5.6.0.6, decisione del titolare, incarico 26)

In architettura si era deciso (01 §5.6.0.6, RDA sulla chiusura automatica) che la giornata si chiude AUTOMATICAMENTE quando tutti i gruppi hanno concluso, e che **non esiste alcun comando di fine giornata**: la chiusura è la conseguenza dell'ultimo ordine del giocatore, non un'iniziativa. Il titolare, giocando la build 27, ha incontrato un secondo blocco che non sa riprodurre. **Decisione del titolare, registrata come modifica voluta e non come correzione di una svista:** esista un comando esplicito per chiudere la giornata e proseguire. La motivazione è la sua: il titolare non può restare bloccato da un difetto in una partita in corso — deve avere una via d'uscita che non dipenda dal fatto che il difetto sia stato trovato. `MotoreCampagna.chiudiLaGiornataForzata` chiude la giornata quale che sia lo stato dei gruppi, marcando SPESA la giornata di ogni gruppo non-agito (la chiusura del titolare vale, per essi, come un ordine di restare), poi lascia proseguire la cascata automatica (`chiudiLaGiornataSeServe`). `SessioneCampagna.chiudiGiornata` la iscrive nel giornale (caso nuovo `giornataChiusaDalGiocatore`, schema 7→8), sicché SOPRAVVIVE a un riavvio: rigiocando, la giornata resta chiusa. Sulla mappa (`SchermataMappaCampagna`) è il pulsante «Chiudi la giornata», sempre fra i comandi globali (accessibilityElements), annunciato con significato di conferma e SENZA rubare il fuoco (nessun `Fuoco.sposta`, come annulla e azzera). Una battaglia in sospeso NON si scavalca (01 §6.4): quel blocco si scioglie aprendo la battaglia dalla casella. **La chiusura automatica RESTA e non è tolta: il comando si aggiunge, non la sostituisce.** Il comando NON è un rimedio ai blocchi — è una via d'uscita per il giocatore —: ogni blocco trovato va corretto alla radice (la battaglia che non si concludeva, RDA-140), e il tasto non lo maschera.

### RDA-139 — La registrazione diagnostica della chiusura, accanto al salvataggio (incide sul gioco; incarico 26)

Quando il comando di chiusura è premuto in una situazione in cui la giornata NON si sarebbe chiusa da sé — c'erano gruppi non-agiti —, il gioco REGISTRA il fatto: `SessioneCampagna.chiudiGiornata` ritorna una `DiagnosticaChiusura` con, per ogni gruppo non-agito, il numero, il nome, la parte, lo stato dichiarato, e **le azioni che gli erano disponibili** (calcolate come l'invariante della giocabilità, `azioniDisponibili`: presidio, sosta, marcia, esplorazione, imboscata secondo la categoria — o NESSUNA, che è il blocco colto sul fatto). **Dove vive:** `PartitaCampagna.chiudiGiornata` la scrive in `diagnostica-giornata-N.json`, nello STESSO slot di campagna accanto a `giornale.jsonl` e alle istantanee (`PartitaCampagna.cartellaCampagna`), con scrittura atomica. **Come si recupera:** il titolare manda indietro la cartella dello slot — il file viaggia col salvataggio, perché è lì dentro. Non è un canale del collaudo: lo scrive il gioco vero (la Presentazione), quando il titolare preme il tasto. È l'unico modo di trasformare un blocco che il titolare non sa riprodurre in un dato utilizzabile: se un gruppo risulta non-agito e con `azioniDisponibili` vuoto, quello è il blocco, colto e conservato. Il giornale NON è stato scelto come casa della diagnostica (resterebbe un vocabolario stretto di comandi e marcatori, versionato e rigiocato): un file a sé, della stessa specie delle istantanee, non tocca lo schema di rigiocatura.

### RDA-140 — La battaglia sbilanciata che non si concludeva: la riserva non schierabile non tiene «viva» la parte (incide sul gioco; 01 §15.2.3, §8.6, incarico 26)

La sessione dell'incarico 25 aveva segnalato di sua iniziativa, senza toccarla, che una battaglia molto sbilanciata non si concludeva mai. **Causa vera:** l'annientamento (01 §15.2.3) contava viva una parte finché il suo deck aveva un esemplare da giocare — `!(deck[parte]).contains { $0.esemplari > 0 }` —, anche quando quell'esemplare era una riserva TROPPO GRANDE per il budget e quindi mai schierabile (01 §8.6: il costo di un piazzamento è almeno il volume di uno sciame; il budget per turno è al più `budgetVolumeBase + quotaRiporto·budgetVolumeBase`, o il budget del primo turno, e non cresce oltre — `MotoreBattaglia.concludiTurno`). Senza sciami in campo, con una riserva che non poteva entrare, la parte non veniva mai annientata; e col tattico che, perdente, non dichiara resa (la propensione, gonfiata dal vantaggio nascosto, può essere irraggiungibile), nessuna delle vie previste chiudeva lo scontro — turni all'infinito. Nel percorso di campagna non c'è tetto ai giri, sicché lo scontro RESTAVA APERTO: un candidato forte del secondo blocco del titolare (bloccato nella schermata di battaglia). **Correzione, per costruzione:** `MotoreBattaglia.haRiserveSchierabili` confronta il volume di uno sciame di ciascun elemento del deck col budget massimo residuo (`budgetMassimoResiduo`); una riserva che lo eccede NON tiene viva la parte. Senza sciami in campo e senza riserve schierabili, la parte è annientata (01 §15.2.3, una via prevista), e lo scontro si chiude in pochi giri. Nessun valore del combattimento toccato (corpo a corpo, soglie di disingaggio, danni intatti): cambia solo il criterio di «parte vuota». **Invariante con mutante:** `SondaInvariantiCampagna.controllaConclusioneBattaglia(concluso:giri:limite:)` (`battaglia_non_conclusa_nel_limite`) pretende che ogni battaglia si concluda entro un limite dichiarato di giri; mutante in `tavolaDeiMutanti`. Visto fallire di proposito sul criterio vecchio: `test_incarico_26_la_battaglia_sbilanciata_si_conclude` con `haRiserveSchierabili` riportato alla forma pre-correzione dà «non conclusa entro 300 giri (giro=301)» e l'invariante scatta `battaglia_non_conclusa_nel_limite:giri=301:limite=300`; verde con la correzione.
