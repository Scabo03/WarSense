...
**Prima cosa mancante: la catena intera.** L'incarico chiedeva, come verifica più importante, che una partita giocata dall'interfaccia dall'inizio alla fine desse lo stesso stato della medesima sequenza di comandi applicata direttamente. La prova è stata scritta in quattro forme successive e nessuna è passata. Il fatto misurato, per chi la riprende: `VistaACaselle.attivaAlTocco` sulla casella di destinazione di una marcia designata restituisce `true` — dunque l'elemento è risolto e `accessibilityActivate` è stato invocato — ma `schermata.statoPerProva?.gruppi[id]?.azioneSpesa` non diventa vero entro cinque secondi di attesa. Le stesse marce ordinate con `PartitaCampagna.esegui` funzionano. Il sospetto non verificato è che lo stato che la schermata conserva sia più vecchio di quello della Sessione nel momento in cui `comandoDiMarcia` calcola il costo in giorni, e che il comando venga quindi respinto per `costoNonPrescritto`; **non è stato accertato** e non va riportato come se lo fosse. Il primo passo per chiunque riprenda è leggere il motivo del rifiuto, che `eseguiComando` oggi annuncia e non registra.

**Seconda cosa mancante: il tocco sintetizzato su una griglia scorrevole.** La prova gemella sul piano della battaglia — selezionare una tessera del deck e schierare toccando una cella della zona arretrata — non passa attraverso XCUITest. Accertato con tre sonde, e va distinto ciò che è stato escluso da ciò che resta ignoto.

*Escluso.* Non è il riconoscitore: una sonda in processo mostra che `finestra.hitTest` sul centro della cella di riga 8 restituisce `VistaGriglia` e che `elemento(sotto:)` la risolve. Non è `delaysContentTouches`: portato a falso, nulla cambia, e la modifica è stata ritirata perché nessuna prova la giustificava. Non è la cornice fuori dalla finestra: la prova filtra ora le celle sulla porzione di schermo occupata dalla griglia.

*Emerso per strada, e vero.* La cornice che il servizio di accessibilità riporta per una casella fuori dalla porzione visibile è la sua posizione nel CONTENUTO, non sullo schermo: sulla mappa grande, il centro della casella del proprio gruppo cade dove è disegnato un comando globale, e `hitTest` vi restituisce un `UIButton`. Il dito, a differenza della voce, non ha scorrimento automatico — la voce che arriva su una casella fuori vista la porta in vista da sé (00 §10.4), il dito deve scorrere. Non è una violazione di 02 §2.11, perché tutto resta raggiungibile in entrambi i modi; è una asimmetria di comodità che vale la pena provare sul dispositivo.

*Ignoto.* Perché un tocco sintetizzato da XCUITest su una cella dentro la porzione visibile di una griglia scorrevole non produca l'effetto, mentre lo produce sulla mappa quattro per quattro, che scorrevole non è.
