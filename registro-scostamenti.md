...
**Seconda cosa mancante: il tocco sintetizzato su una griglia scorrevole — meccanismo TROVATO, caso della battaglia ancora aperto (2026-08-05).**

*Il meccanismo, dimostrato.* La cornice che il servizio di accessibilità riporta per una casella fuori dalla porzione visibile di un contenitore scorrevole è la sua posizione nel CONTENUTO e non sullo schermo. Può quindi cadere dentro la finestra, e perfino sopra la colonna dei comandi, senza che in quel punto sia disegnata la casella: toccarla non fa nulla, perché il dito colpisce ciò che è davvero disegnato lì. Misurato sulla mappa grande, che è scorrevole quanto la griglia di battaglia: la casella del proprio gruppo riportava cornice a y=592 con i comandi a y=623 — dunque superava il filtro «dentro la finestra e sopra i comandi» — e il tocco non apriva nulla; dopo UN solo scorrimento la cornice passava a y=475 e il tocco apriva il pannello. Non è un difetto del riconoscitore né del gioco: il dito, a differenza della voce, non ha scorrimento automatico verso l'elemento. **Il punto è chiuso come conseguenza nota per la mappa di campagna.**

*Ipotesi escluse, con la prova.* (a) Il riconoscitore: sonda in processo, `finestra.hitTest` sul centro della cella di riga 8 restituisce `VistaGriglia` ed `elemento(sotto:)` la risolve. (b) `delaysContentTouches`: portato a falso senza effetto, modifica ritirata perché nessuna prova la giustificava. (c) La cornice fuori vista come sola causa: esclusa per la battaglia dal fatto che segue.

*Che cosa resta aperto.* Sulla griglia di battaglia il rimedio dello scorrimento non basta: portando in vista una cella della zona di schieramento — otto tentativi con scorrimenti sulla finestra e otto con scorrimenti dentro la griglia — il tocco non schiera. Il fenomeno della battaglia non coincide dunque con quello della mappa, o non del tutto, e la parte residua resta ignota. La differenza fra i due casi è ora però stretta: due griglie scorrevoli, stesso riconoscitore, stessa base `VistaACaselle`, e la mappa risponde mentre la battaglia no. Chi riprende ha questo come punto di partenza.

*Conseguenza possibile sul gioco, non realizzata.* Se la cornice riportata è la posizione nel contenuto anche per gli elementi fuori vista, allora l'esplorazione al tatto (00 §8.3) può, appoggiando il dito sopra la colonna dei comandi, trovarvi una casella che lì non è disegnata. **Non verificato**: richiede VoiceOver reale e non è osservabile qui. Il rimedio sarebbe riportare la cornice soltanto per gli elementi dentro la porzione visibile; la sua portata non è nulla, perché `RaggiungibilitaTest` pretende cornici non degeneri per ogni elemento del percorso e andrebbe riformulato. Iscritto fra le verifiche possibili solo su dispositivo, non realizzato.
